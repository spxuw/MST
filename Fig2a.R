...
# vary the identity
for (C in c(0,0.00001,0.0001,0.001,0.01,0.1,0.2)){
  set.seed(123)
  N = 90
  M = 3
  #C = 0.5
  ...
  for (rea in 1:realization){
    ...
    corr_R = c()
    for (i in 1:M){
      data_sub = data.frame(true_contribution = true_source[,i], predicted_contribution = as.numeric(FEAST_output[,i]))
      data_sub.lm = lm(true_contribution~predicted_contribution, data = data_sub)
      corr_R = c(corr_R, summary(data_sub.lm)$r.squared )
    }
    corr_feast = c(corr_feast, mean(corr_R))
    ...
    corr_R = c()
    for (i in 1:(M)){
      data_sub = data.frame(true_contribution = true_source[,i], predicted_contribution = as.numeric(results_pro[,i]))
      data_sub.lm = lm(true_contribution~predicted_contribution, data = data_sub)
      corr_R = c(corr_R, summary(data_sub.lm)$r.squared )
    }
    corr_sourcetracker = c(corr_sourcetracker, mean(corr_R))
    ...
    corr_R = c()
    for (i in 1:(M)){
      data_sub = data.frame(true_contribution = true_source[,i], predicted_contribution = as.numeric(p2[,i]))
      data_sub.lm = lm(true_contribution~predicted_contribution, data = data_sub)
      corr_R = c(corr_R, summary(data_sub.lm)$r.squared )
    }
    corr_randomforest = c(corr_randomforest, mean(corr_R))
  }
  corr_final = c(corr_final, mean(corr_feast), mean(corr_sourcetracker), mean(corr_randomforest))
  strength_final = c(strength_final, C, C, C)
...
g1<- ggplot(dat, aes(x=strength_final+0.000001, y=corr_final,color=method)) +
  ...
  scale_color_jcolors(palette = "pal5") + theme_bw()+xlab("Connectivity") + ylab(R^2~'of proportion estimates')+
...
ggsave(g1,file="../figures/GLV_linear_interaction_C_relative.pdf",width=3, height=2.8)
write.table(dat, file = "../results/GLV/R_linear_interaction_C_relative.csv", row.names = FALSE, col.names = FALSE,sep=",")
